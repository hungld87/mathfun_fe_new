...
const currentCategory = computed(() => {
  ...
  return categories.value.find((cat: any) => cat.slug === categorySlug)
})
...
const filteredNews = computed(() => {
  ...
  return allNews.value.filter((news: any) => news.categorySlug === categorySlug)
})
  ...
  <main class="bg-background">
    <div class="container mx-auto px-6 py-10 max-w-content">
      <h1 class="text-3xl font-bold text-primary mb-8">
        Tin Tức
        <span v-if="currentCategory" class="text-gray-600"> · {{ currentCategory.name }}</span>
      </h1>
        ...
        <div class="flex items-center gap-3 flex-wrap">
          ...
          <NuxtLink 
            ...
            :class="[
              ...
              'bg-white border border-gray-200 text-gray-700 hover:border-teal-500 hover:text-teal-600 hover:bg-teal-50'
            ]"
          >
            Tất cả
          </NuxtLink>
      ...
      <!-- News List -->
      <div v-if="displayedNews.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <NewsCard 
      ...
      <div v-else class="text-center py-12">
        <p class="text-gray-500 text-lg">Chưa có bài viết nào trong danh mục này</p>
      </div>
